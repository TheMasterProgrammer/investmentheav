...
class BinanceClient:
    ...
    def getHistoricalData(self, asset, intervalBC, interval1, datapoints=3, mean_vol=10, MA_LENGTH=3000):
        ...
        df['Upper'] = df['MA20'] + 2.5 * df['stddev']
        df['Lower'] = df['MA20'] - 2.5 * df['stddev']
    ...
    def get_futures_account_usdt(self):
        balance = self.client.futures_account()["assets"][6]["availableBalance"]
        print(balance)
        return balance
    ...
    def get_precision(self, symbol):
        ...
        for x in info['symbols']:
            if x['symbol'] == symbol:
                print("PRECISION FOUND: {}".format(x['quantityPrecision']))
                return x['quantityPrecision']
    def get_usdt_precision(self, symbol):
        info = self.client.futures_exchange_info()
        for x in info['symbols']:
            if x['symbol'] == symbol:
                print("PRECISION FOUND: {}".format(x['quantityPrecision']))
    ...
    def enter_short(self, asset, leverage, pctg):
        symbol = asset + 'USDT'
        precision = self.get_precision(symbol)
        self.client.futures_change_leverage(symbol=symbol, leverage=leverage)
        _qty = float(self.get_futures_account_usdt()) * pctg
        print("balance: {}".format(_qty))
        _qty = _qty/self.getPrice(asset)
        print("precision: {}".format(precision))
        print("quantity: {}".format(_qty))
        qty = math.trunc(float(_qty * 10 ** precision)) / 10 ** precision
        short_order = {
            "symbol": symbol,
            "side": "SELL",
            "type": "MARKET",
        ...
        print("Quantity of {} sold: {}".format(asset, qty))
        order = self.client.futures_create_order(**short_order)
        print(order['origQty'])
        ...
        self.client.futures_change_leverage(symbol=symbol, leverage=leverage)
        precision = self.check_decimals(symbol)
        _qty = float(self.get_futures_account_usdt())
        price = self.getPrice(asset)
        order_size = (price/_qty)*pctg
        qty = math.trunc(float(order_size * 10 ** precision)) / 10 ** precision
        order = self.client.futures_create_order(symbol=symbol, side='SELL', type='MARKET', quoteQty=qty)
        print(order)
        '''
    def short_take_profit(self, asset, qty, price):
        symbol = asset + 'USDT'
        print("original price: {}".format(price))
        precision = self.get_usdt_precision(symbol)
        price = math.trunc(float(price * 10 ** precision)) / 10 ** precision
        print("final_price: {}".format(price))
        short_order = {
            "symbol": symbol,
            "side": "BUY",
        ...
        #print("Quantity of {} bought: {}".format(asset, qty))
        order = self.client.futures_create_order(**short_order)
        print(order)
    def exit_short(self, asset, qty):
        symbol = asset + 'USDT'
        short_order = {
            "symbol": symbol,
            "side": "BUY",
        ...
        precision = self.check_decimals(symbol)
        qty = math.trunc(float(balance * 10**precision))/10**precision
        order = self.client.futures_create_order(symbol=symbol, side='BUY', type='MARKET', qty=qty)
        print(order)
        '''
